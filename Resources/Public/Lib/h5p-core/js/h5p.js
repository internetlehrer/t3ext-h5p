  ...
  H5P.jQuery('.h5p-content:not(.h5p-initialized)', target).each(function () {
    ...
    H5P.getUserData(contentId, 'state', function (err, previousState) {
      for (let i = 0; i < sessionStorage.length; i++) {
        if (sessionStorage.key(i).includes(location.pathname + "/h5pcid_" + contentId) && sessionStorage.key(i).includes("h5p-state___")) H5P["previous" + contentId] = sessionStorage.getItem(sessionStorage.key(i));
      }
      if (H5P["previous" + contentId]) previousState = JSON.parse(H5P["previous" + contentId]);
      if (previousState) {
        library.userDatas = {
          state: previousState
        };
...
(function ($) {
  ...
  H5P.setUserData = function (contentId, dataId, data, extras) {
    ...
    if (H5P.instances[0].libraryInfo.machineName !== "H5P.Questionnaire") sessionStorage.setItem("h5p-state___" + location.pathname + "/h5pcid_" + contentId, data);
  };
